...
const getDramas = async (
  ...
  try {
    const allDramas = await dramas.find({}, { noCursorTimeout: false })
      .toArray();
...
const getDrama = async (
  ...
  try {
    const drama = await dramas.findOne({ _id: new Bson.ObjectId(params.id) }, {
...
const updateDrama = async (
  ...
  try {
    ...
    await dramas.updateOne({ _id: new Bson.ObjectId(params.id) }, {
      $set: newDramaValues,
    });
    response.body = {
      success: true,
      notice: "Drama was successfully updated!",
    };
  } catch (error) {
    response.body = { success: false, error };
  }
...
const deleteDrama = async (
  ...
  try {
    ...
    response.body = {
      success: true,
      notice: "Drama was successfully deleted!",
    };
  } catch (error) {
    response.body = { success: false, error };
  }
...
export { addDrama, deleteDrama, getDrama, getDramas, updateDrama };
